...
def train_and_validate(class_dirs, save_path, num_epochs, device, log_method):
    ...
    for epoch in range(num_epochs):
        ...
        log_method(f'Epoch {epoch+1}/{num_epochs}, Train Loss: {avg_train_loss}')

        val_loss, val_accuracy = evaluate_model(model, val_loader, criterion, device)
        log_method(f"Epoch {epoch + 1}/{num_epochs}, Validation Loss: {val_loss}, Val Accuracy: {val_accuracy}'")

        if val_loss < best_val_loss:
            best_val_loss = val_loss
            torch.save(model.state_dict(), save_path)
            log_method(f"Model saved at epoch {epoch + 1} with validation loss: {best_val_loss}")

    log_method('End training')
    return model
